...
async def infobot(ctx):
  ...
  embed.set_footer(text="version 2 pre-release, discord.py", icon_url=botavatar)
...
async def help(ctx):
  ...
  embed.add_field(name="IMAGE COMMANDS", value="```dog```, ```cat```, ```pat```, ```hug```, ```wink```")
...
async def changelog(ctx):
  ...
  embed.set_thumbnail(url="https://cdn.discordapp.com/emojis/827509069366427668.png")
  embed.add_field(name="VERSION 2.0 (huge update)", value="REVAMPED ALOT OF STUFF (cool), ADDED IMAGES COMMANDS (check in help command), BECAME LESS RACIST (no balls), ORGANIZED CODE FILE (might open source this lol)")
